...
export const researchTools: Tool[] = [
  {
    ...
    function: {
      name: "search_web",
      description:
        "搜索互联网资料。只要用户明确要求查询资料、搜索文档、联网查信息、查公开情报、查组件说明、查漏洞说明，就应立即优先调用该工具。如果首轮结果不够相关，必须继续自主改写关键词重试，可加入 site:域名、官方站点名、年份、版本号、城市名、引号短语等约束，而不是向用户索要链接。",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "fetch_webpage",
      description:
        "抓取网页正文。拿到搜索结果后，如需阅读具体页面内容、提取说明、整理资料摘要，应继续调用该工具。若首个页面不够相关，应返回搜索结果继续选择其他候选页，而不是直接放弃。",
      parameters: {
        type: "object",
        properties: {
