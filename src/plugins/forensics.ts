...
export const forensicsCommands: PluginCommand[] = [
  ...
  },
  {
    ...
    category: 'response',
    subCategory: 'live',
    subCategoryLabel: '实时态势',
    subCategoryLabelEn: 'Live Telemetry',
    name: 'Network Byte Counters',
    cn_name: '网络字节计数器',
    description: 'Collect RX/TX byte counters for realtime traffic trend',
    cn_description: '采集网卡 RX/TX 字节计数，用于实时流量趋势',
    command: "awk 'NR>2 {rx+=$2; tx+=$10} END {printf \"RX:%d TX:%d\\n\", rx, tx}' /proc/net/dev",
    icon: Network,
    parserType: 'raw'
  },
  {
    ...
    subCategoryLabel: '实时态势',
    subCategoryLabelEn: 'Live Telemetry',
    name: 'Established Connection Count',
    cn_name: '已建立连接计数',
    description: 'Count established TCP connections',
    cn_description: '统计当前已建立 TCP 连接数量',
    command: "(ss -ant state established 2>/dev/null || netstat -ant 2>/dev/null | grep ESTABLISHED || true) | awk 'NR>1 {c++} END {printf \"CONN:%d\\n\", c+0}'",
    icon: ShieldCheck,
    parserType: 'raw'
  },
  {
    ...
    name: 'Host CPU Usage',
    cn_name: '主机 CPU 占用',
    description: 'Collect current host CPU usage percentage',
    cn_description: '采集当前主机 CPU 使用率',
    command: "top -bn1 | awk -F',' '/Cpu\\(s\\)|%Cpu/ {for(i=1;i<=NF;i++){if($i ~ / id/){gsub(/[^0-9.]/,\"\",$i); printf \"CPU:%.1f\\n\", 100-$i; exit}}}'",
    icon: Cpu,
    parserType: 'raw'
  },
  {
    ...
    cn_name: '主机内存占用',
    description: 'Collect current memory usage percentage',
    cn_description: '采集当前主机内存使用率',
    command: "free | awk '/Mem:/ {printf \"MEM:%.1f\\n\", ($3/$2)*100}'",
    icon: HardDrive,
    parserType: 'raw'
  },
  {
    ...
    category: 'response',
    subCategory: 'network',
    subCategoryLabel: '流向分析',
    subCategoryLabelEn: 'Flow Analysis',
    name: 'Active Flow Destinations',
    cn_name: '活跃连接去向',
    description: 'List active TCP flow destinations with process hint',
    cn_description: '列出活跃 TCP 连接去向与进程线索',
    command: "ss -tunp 2>/dev/null | awk 'NR>1 {local=$5; remote=$6; proto=$1; proc=$0; sub(/^.*users:/,\"\",proc); split(local,l,\":\"); split(remote,r,\":\"); lip=l[1]; lport=l[length(l)]; rip=r[1]; rport=r[length(r)]; if(rip != \"\" && rip != \"*\") print lip\"|\"lport\"|\"rip\"|\"rport\"|\"proto\"|\"proc}' | head -n 80",
    icon: Globe,
    parserType: 'raw'
  },
  {
    ...
    category: 'response',
    subCategory: 'network',
    subCategoryLabel: '暴露面',
    subCategoryLabelEn: 'Exposure',
    name: 'Listening Services',
    cn_name: '监听服务暴露面',
    description: 'List listening ports and owning processes',
    cn_description: '列出监听端口与对应进程',
    command: "ss -tunlp 2>/dev/null | awk 'NR>1 {proto=$1; local=$5; proc=$0; sub(/^.*users:/,\"\",proc); split(local,l,\":\"); port=l[length(l)]; print proto\"|\"port\"|\"proc}' | head -n 80",
    icon: Network,
    parserType: 'raw'
  },
  {
    ...
    category: 'response',
    subCategory: 'process',
    subCategoryLabel: '资源进程',
    subCategoryLabelEn: 'Resource Processes',
    name: 'Top CPU Processes',
    cn_name: 'CPU 高占用进程',
    description: 'Top processes by CPU usage',
    cn_description: '按 CPU 占用排序的进程',
    command: "ps -eo pid=,comm=,%cpu=,%mem= --sort=-%cpu | head -n 30 | awk '{pid=$1; cpu=$(NF-1); mem=$NF; $1=\"\"; $(NF-1)=\"\"; $NF=\"\"; sub(/^ +/,\"\"); print pid\"|\"$0\"|\"cpu\"|\"mem}'",
    icon: Cpu,
    parserType: 'raw'
  },
  {
    ...
    cn_description: '按内存占用排序的进程',
    command: "ps -eo pid=,comm=,%cpu=,%mem= --sort=-%mem | head -n 30 | awk '{pid=$1; cpu=$(NF-1); mem=$NF; $1=\"\"; $(NF-1)=\"\"; $NF=\"\"; sub(/^ +/,\"\"); print pid\"|\"$0\"|\"cpu\"|\"mem}'",
    icon: HardDrive,
    parserType: 'raw'
  },
  {
